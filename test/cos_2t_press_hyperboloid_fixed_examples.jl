...
function _execute(n = 8, thickness = Length/2/100, visualize = false, distortion = 0.0)
    ...
    visualize && vtkwrite("cos_2t_press_hyperboloid_fixed-$(n)-$(thickness)-$(distortion)-m.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
    scalars = []
    for nc in 1:3
    ...
    visualize && vtkwrite("cos_2t_press_hyperboloid_fixed-$(n)-$(thickness)-$(distortion)-n.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
    scalars = []
    for nc in 1:2
    ...
    visualize && vtkwrite("cos_2t_press_hyperboloid_fixed-$(n)-$(thickness)-$(distortion)-q.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
...
let
    ...
    for distortion in [2.0, 0.0]
        ns, results100 = cos_2t_press_hyperboloid_fixed_examples.test_convergence(1/100, distortion)
        ns, results1000 = cos_2t_press_hyperboloid_fixed_examples.test_convergence(1/1000, distortion)
        ns, results10000 = cos_2t_press_hyperboloid_fixed_examples.test_convergence(1/10000, distortion)
        ns, results100000 = cos_2t_press_hyperboloid_fixed_examples.test_convergence(1/100000, distortion)
