# Pressurized hyperboloid clamped around the end circular section.
# 
...
module cos_2t_press_hyperboloid_fixed_q4rnt_examples
...
function _execute(n = 8, thickness = Length/2/100, visualize = false, distortion = 0.0)
    formul = FinEtoolsFlexStructures.FEMMShellQ4RNTModule
    tolerance = Length/n/100
    fens, fes = distortblock(Q4block, 90/360*2*pi, Length/2, n, n);
    fens.xyz = xyz3(fens)
    ...
    visualize && vtkwrite("cos_2t_press_hyperboloid_fixed_q4rnt-$(n)-$(thickness)-$(distortion)-m.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
    scalars = []
    for nc in 1:3
    ...
    visualize && vtkwrite("cos_2t_press_hyperboloid_fixed_q4rnt-$(n)-$(thickness)-$(distortion)-n.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
    scalars = []
    for nc in 1:2
    ...
    visualize && vtkwrite("cos_2t_press_hyperboloid_fixed_q4rnt-$(n)-$(thickness)-$(distortion)-q.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
...
function test_convergence(thicknessmult = 1/100, distortion = 0.0)
    println("--------------------------------------------------------------------------------")
    @info "Pressurized hyperboloid of rotation, fixed ends, thickness multiplier = $(thicknessmult), distortion = $(distortion)"
    results = []
    ns = [16, 32, 64,]
    for n in ns
...
using .cos_2t_press_hyperboloid_fixed_q4rnt_examples

let

    for distortion in [2.0, 0.0]
        ns, results100 = cos_2t_press_hyperboloid_fixed_q4rnt_examples.test_convergence(1/100, distortion)
        ns, results1000 = cos_2t_press_hyperboloid_fixed_q4rnt_examples.test_convergence(1/1000, distortion)
        ns, results10000 = cos_2t_press_hyperboloid_fixed_q4rnt_examples.test_convergence(1/10000, distortion)
        ns, results100000 = cos_2t_press_hyperboloid_fixed_q4rnt_examples.test_convergence(1/100000, distortion)
