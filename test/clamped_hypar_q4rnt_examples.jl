...
"""
module clamped_hypar_q4rnt_examples
...
function _execute_half(tL_ratio = 1/100, n = 32, visualize = false)
    # Parameters:
    E = 2.0e11 * phun("Pa");
    nu = 0.3;
    L = 1.0 * phun("m")
    ...
    lfemm = FEMMBase(IntegDomain(fes, GaussRule(2, 2), thickness))
...
function test_convergence()
    ...
    for (tL_ratio, ) in zip(tL_ratios, )
        ...
        seresults = Float64[]
        for n in ns
            u, se = _execute_half(tL_ratio, n, false)
            push!(uresults, u)
            push!(seresults, se)
        end   
...
using .clamped_hypar_q4rnt_examples
using FinEtools

let
    
    ns, all_uresults, all_seresults = clamped_hypar_q4rnt_examples.test_convergence()
    ...
    for i in 1:3
        ...
        @show qtrue = (q2^2 - q1 * q3) / (2 * q2 - q1 - q3) / phun("m")
        q1, q2, q3 = all_seresults[i][end-2:end]
        @show qtrue = (q2^2 - q1 * q3) / (2 * q2 - q1 - q3) / phun("N*m")
    end
